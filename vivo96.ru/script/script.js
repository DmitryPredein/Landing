let arrayModel =  [['Телефон', 'NEX 3'],['Телефон', 'V11'],['Телефон', 'NEX Dual Display Edition'],['Телефон', 'Z5X'],['Телефон', 'Y95'],['Телефон', 'Y91C'],['Телефон', 'Y81'],['Телефон', 'Y66'],['Телефон', 'Y17'],['Телефон', 'Y11'],['Телефон', 'XPlay 6'],['Телефон', 'Xplay 5'],['Телефон', 'X9s Plus'],['Телефон', 'X9 Plus'],['Телефон', 'X9'],['Телефон', 'X7'],['Телефон', 'X6S'],['Телефон', 'X6 Plus'],['Телефон', 'X6'],['Телефон', 'X21 UD'],['Телефон', 'X21'],['Телефон', 'X20 Plus UD'],['Телефон', 'X20 Plus'],['Телефон', 'V9'],['Телефон', 'V7'],['Телефон', 'V5 Plus'],['Телефон', 'V5 Lite'],['Телефон', 'V5'],['Телефон', 'V17 Neo'],['Телефон', 'V17'],['Телефон', 'V15 Pro'],['Телефон', 'V11i'],];
...
$(document).ready( function () {
  ...
  function sendAjaxForm(url) {
      $.ajax({
          ...
          data: "name="+$("#firstNameId").val()+" "+$("#nameId").val()+" "+$("#lastNameId").val()+"&tel="+$("#telId").val()+"&nodeJob="+nodeJob_status+"&desp="+$("#descId").val()+"&type="+priceDevice_check+"&brand=Vivo&model="+priceModel_check+"&service="+$("#service_title").text()+"&priceService="+price_check,  // Сеарилизуем объект
  ...
  function sendAjaxFormConsult(url, name, tel) {
      $.ajax({
          ...
          type:     "POST", //метод отправки
          dataType: "html", //формат данных
          data: "name="+name+"&tel="+tel+"&brand=Vivo&nodeJob="+nodeJob_status,  // Сеарилизуем объект
 ...
 function someFunction(data) {
  ...
  arrayModel.forEach((item, i) => {
    $('.modelMainContent section').append('<div class="modelBox"><span class="titleModelBox">'+item[1]+'</span>'+'<img src="img/Vivo/vivo/'+item[1].trim().toLowerCase().replace(/[^a-zA-Z0-9 -]/, "").replace(/\s/g, "-")+'.png" class="img_modelBox"></div>');
  });
