let arrayModel =  [['Телефон', 'T29'],['Телефон', 'R9s Plus'],['Телефон', 'Reno'],['Телефон', 'Reno 10x Zoom'],['Телефон', 'Reno Z '],['Телефон', 'RX17 Neo'],['Телефон', 'RX17 Pro'],['Телефон', 'A1K'],['Телефон', 'A3s'],['Телефон', 'A5'],['Телефон', 'A5s'],['Телефон', 'A83'],['Телефон', 'AX7'],['Телефон', 'Clover R815'],['Телефон', 'F3 Plus'],['Телефон', 'F5'],['Телефон', 'F7'],['Телефон', 'F9'],['Телефон', 'Find 5'],['Телефон', 'Find 7'],['Телефон', 'Find 7a'],['Телефон', 'Find X'],['Телефон', 'Finder'],['Телефон', 'Mirror R819'],['Телефон', 'Muse R821'],['Телефон', 'N1'],['Телефон', 'N3'],['Телефон', 'R11'],['Телефон', 'R11s'],['Телефон', 'R15'],['Телефон', 'R15 Dream Mirror Edition'],['Телефон', 'R5'],['Телефон', 'R7s'],['Телефон', 'R9s'],];
...
$(document).ready( function () {
  ...
  function sendAjaxForm(url) {
      $.ajax({
          ...
          data: "name="+$("#firstNameId").val()+" "+$("#nameId").val()+" "+$("#lastNameId").val()+"&tel="+$("#telId").val()+"&nodeJob="+nodeJob_status+"&desp="+$("#descId").val()+"&type="+priceDevice_check+"&brand=Oppo&model="+priceModel_check+"&service="+$("#service_title").text()+"&priceService="+price_check,  // Сеарилизуем объект
  ...
  function sendAjaxFormConsult(url, name, tel) {
      $.ajax({
          ...
          type:     "POST", //метод отправки
          dataType: "html", //формат данных
          data: "name="+name+"&tel="+tel+"&brand=Oppo&nodeJob="+nodeJob_status,  // Сеарилизуем объект
 ...
 function someFunction(data) {
  ...
  arrayModel.forEach((item, i) => {
    $('.modelMainContent section').append('<div class="modelBox"><span class="titleModelBox">'+item[1]+'</span>'+'<img src="img/Oppo/oppo/'+item[1].trim().toLowerCase().replace(/[^a-zA-Z0-9 -]/, "").replace(/\s/g, "-")+'.png" class="img_modelBox"></div>');
  });
